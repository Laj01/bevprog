...
public:
   ...
   */
    ...
    {
        melyseg = 0;
        kiir_pre (&gyoker, std::cout);
    }
    void kiir_post (void)
    {
        melyseg = 0;
        kiir_post (&gyoker, std::cout);
    }
    void kiir_in (void)
    {
        melyseg = 0;
        kiir_in (&gyoker, std::cout);
    }
  ...
  */
    ...
    void kiir_in (std::ostream & os)
    {
        melyseg = 0;
        kiir_in (&gyoker, os);
    }

    void kiir_pre (std::ostream & os)
    {
        melyseg = 0;
        kiir_pre (&gyoker, os);
    }

    void kiir_post (std::ostream & os)
    {
        melyseg = 0;
        kiir_post (&gyoker, os);
    }
...
private:
    ...
    void kiir_in (Csomopont * elem, std::ostream & os)
    {
        if (elem != NULL)
        {
            ++melyseg;
            kiir_in (elem->nullasGyermek (), os); //bal-subtree
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            kiir_in (elem->egyesGyermek (), os); //jobb-subtree
            --melyseg;
        }
    }

    void kiir_pre (Csomopont * elem, std::ostream & os)
    {
        if (elem != NULL)
        {
            ++melyseg;
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            kiir_pre (elem->nullasGyermek (), os); //bal-subtree
            kiir_pre (elem->egyesGyermek (), os); //jobb-subtree
            --melyseg;
        }
    }

    void kiir_post (Csomopont * elem, std::ostream & os)
    {
        if (elem != NULL)
        {
            ++melyseg;
            kiir_post (elem->nullasGyermek (), os); //bal-subtree
            kiir_post (elem->egyesGyermek (), os); //jobb-subtree
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            --melyseg;
    ...
    void kiir (Csomopont * elem, std::ostream & os) //az eredeti fabejárás
    {
        if (elem != NULL)
        {
            ++melyseg;
...
usage (void)
{
    std::cout << "Usage: lzwtree in_file -o out_file -std/ -in/ -pre/ -post" << std::endl;
}
...
main (int argc, char *argv[])
{
    ...
    std::string clarg = argv[1];  //clarg ==command-line-argumentum


    if(clarg.compare("-std") == 0) //az eredeti
    {
        binFa.kiir(kiFile);
    }
    else if(clarg.compare("-in") == 0)  //inorder
    {
        binFa.kiir_in(kiFile);
    }

    else if(clarg.compare("-pre") == 0) //preorder
    {
        binFa.kiir_pre(kiFile);
    }

     else if(clarg.compare("-post") == 0) //postorder
    {
        binFa.kiir_post(kiFile);
    }
